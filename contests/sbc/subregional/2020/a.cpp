...
int32_t main(){
	...
	double prob = (1.0/(1.0*B-1.0*A+1.0));
	
	double p2 =(1.0*B-1.0*A)/(1.0*B-1.0*A+1.0);
	// cout<<prob<<" "<<p2<<"\n";
	//dpi   * (B-a)/(B-a+1) = 1 + pre[i-A-1] - pre[i-b-1]
	dp[0] = 0;
	pref[0] = 0;
	cout<<setprecision(10)<<fixed;
	for(int i = 1; i <= n; i++){
		if(i <= A and !i) dp[i] = 1.0;
		else{
			if(A == 0){
				dp[i] = (1.0 + (count(i-1) - count(i-B-1))*prob)/p2;
			}
			else dp[i] = 1.0 + prob*( (count(i-A) - count(i-B-1) ));
		}
		// cout<<"kaskdaksd " <<i<<" "<<count(i-A)<<" "<<count(i-B-1)<<"\n";
		// cout<<i<<" "<<dp[i]<<"\n";
